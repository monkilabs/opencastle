...
/** A byte-order mark, as bytes — see `withoutBom` and `markerOffsets`. */
...
export interface MergeResult {
  ...
  /** Markers with no block at all: nothing was written, a person is needed. */
  severed?: boolean
  /**
...
function markerOffsets(content: string, marker: string): number[] {
  // A leading byte-order mark does not make the first line stop being the first
  // line. `withoutBom` was added to the legacy predicates and not here, and this
  // is the reader that decides whether a block exists at all: on a fresh install
  // the root file *is* the block, so `BLOCK_START` sits at offset 0, and three
  // BOM bytes in front of it made `blockRegions` find nothing. `sync` then took
  // the append path and wrote a second complete block — the file doubled to two
  // full instruction sets, every surface went green over it, and `remove --all`
  // left 20KB of this release's own output behind, described as an older
  // release's. The untouched twin, one function away from the fix.
  const bom = content.startsWith(BOM_BYTES)
    ? BOM_BYTES.length
    : content.startsWith('\ufeff')
      ? 1
      : 0
...
export type FileState =
  | 'clean'
  | 'doubled'
  /** A stray marker *beside* an intact block: harmless, and usually documentation. */
  | 'torn'
  /** A stray marker and no block at all: our body is in there with no delimiter. */
  | 'severed'
  | 'unreducible'
  | 'stale'
  | 'unreadable'
...
export function diagnoseManagedFile(
  ...
  //
  // With a block present the file is intact — the block is maintained, the
  // stray is left alone, and the commonest source of that shape is a file
  // documenting the convention. With *no* block, the markers that delimited our
  // body are gone but the body is almost certainly still there: appending
  // writes a second complete instruction set underneath the first, which is
  // what `sync` did — 20KB becoming 40KB, two sets of rules for the assistant,
  // and every surface reporting health because one valid block now existed.
  // Calling both of these "torn" is what let the second inherit the first's
  // "breaks nothing".
  // And only when a body of ours is actually in there. A stray marker with no
  // block *and no generated text* is someone documenting the convention before
  ...
  if (strays > 0 && blocks === 0 && bodyPresent(content)) {
    return {
      state: 'severed',
      fixable: false,
      detail: `has ${strays} OpenCastle marker(s) and no complete block — the text beside them` +
        ' is probably generated output with nothing to delimit it',
      fix:
        'restore the missing marker, or delete the generated text beside it —' +
        ' this one needs a person, and until then sync will not write to the file',
    }
  }
  if (strays > 0) {
    return {
...
export async function writeManagedBlock(path: string, body: string): Promise<MergeResult> {
  ...
  // Never append into a severed file. Our body is in there with its delimiters
  // gone, so a second one goes underneath the first and the assistant reads
  // both. Nothing is written until a person resolves it.
  if (orphanMarkers(current, S, E).length > 0 && carriesLegacyBody(current)) {
    return {
      action: 'unchanged',
